...
/* ****** Read from file ***** */
async function getJSONfromFile<ItemType extends JsonDataType>(filename: filenames): Promise<ItemType[]> {
...
/* ****** Write to file ***** */
async function writeJSONToFile<T extends JsonDataType>(filename: filenames, data: Array<T>): Promise<void> {
  const filePath = path.join(dbPath, filename);
...
/* ****** Delete item ***** */
async function deleteItem<T extends JsonDataType>(filename: filenames, itemId: number): Promise<number> {
  try {
...
export async function getAppointmentsByMonthYear(month: string, year: string): Promise<AppointmentDateMap> {
