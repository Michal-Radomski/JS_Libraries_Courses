...
export function getAppointmentColor(appointmentData: Appointment, userId: number | undefined): [string, string] {
...
export function getAvailableAppointments(appointments: AppointmentDateMap, userId: number | null): AppointmentDateMap {
  ...
  Object.keys(filteredAppointments).forEach((date) => {
    ...
    filteredAppointments[dateNum] = filteredAppointments[dateNum].filter(
      (appointment: Appointment) => (!appointment.userId || appointment.userId === userId) && !appointmentInPast(appointment)
    );
  });
